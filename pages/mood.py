...
MOOD_PROFILES = {
    'Chill': {'energy': 0.3, 'valence': 0.6, 'acousticness': 0.6, 'tempo': 90},
    'Energetic': {'energy': 0.85, 'valence': 0.7, 'acousticness': 0.2, 'tempo': 130},
    'Sad': {'energy': 0.3, 'valence': 0.2, 'acousticness': 0.5, 'tempo': 80},
    'Happy': {'energy': 0.7, 'valence': 0.9, 'acousticness': 0.3, 'tempo': 120},
    'Focus': {'energy': 0.4, 'valence': 0.5, 'acousticness': 0.4, 'instrumentalness': 0.3, 'tempo': 100},
    'Party': {'energy': 0.9, 'valence': 0.8, 'danceability': 0.9, 'tempo': 125},
}
...
def get_mood_recs(mood, contrast_key):
    ...
    for k, v in base.items():
        if k in df_copy.columns and k != 'tempo':
            df_copy['score'] += -(df_copy[k].fillna(0) - v) ** 2
        elif k == 'tempo':
            df_copy['score'] += -((df_copy['tempo'].fillna(100) - v) / 100) ** 2
    recs = df_copy.nlargest(15, 'score').to_dict('records')
    # Normalize to rec shape (track_name, artist, genre)
    out = []
    for r in recs:
        out.append({
...
if st.session_state.mood_view == 'results' and st.session_state.mood_recs:
    ...
    with col2:
        render_rec_card(rec, key_prefix='mood_rec', show_actions=True,
