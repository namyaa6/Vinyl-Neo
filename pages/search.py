    ...
    if search_query:
        with st.spinner('Finding recommendations...'):
            ...
            if len(matches) > 0:
                ...
                # Seed track for left column (enrich with Spotify if possible)
                seed_image = None
                seed_preview = None
                seed_external = None
                if track_id:
...
if all_recs and st.session_state.seed_track:
    ...
    with col_right:
        ...
        if rec.get('features') and st.session_state.get('seed_features'):
    ...
    with col_right:
        ...
        def on_add():
            st.session_state.playlist_queue.append(rec)
            st.info('Added to playlist queue')
        render_rec_card(rec, key_prefix='search_rec', show_actions=True,
